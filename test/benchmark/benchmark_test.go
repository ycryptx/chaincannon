...
var addTests = []addTest{
	...
	{"when amount == 0 duration should be used", endpoint, []string{"/path1"}, 123, 0, 0, benchmark.Recipe{
		Endpoint: endpoint,
		Duration: time.Duration(123) * time.Second,
		Amount:   0,
		Runs:     []benchmark.Run{{TxPaths: []string{"/path1"}}},
	}},
	{"should split runs equally between specified threads", endpoint, []string{"/path1", "/path2"}, 123, 0, 2, benchmark.Recipe{
		Endpoint: endpoint,
		Duration: time.Duration(123) * time.Second,
		Amount:   0,
		Runs:     []benchmark.Run{{TxPaths: []string{"/path1"}}, {TxPaths: []string{"/path2"}}},
	}},
	{"should more than one tx file in same thread if tx_files > threads", endpoint, []string{"/path1", "/path2", "/path3"}, 123, 0, 2, benchmark.Recipe{
		Endpoint: endpoint,
		Duration: time.Duration(123) * time.Second,
		Amount:   0,
		Runs:     []benchmark.Run{{TxPaths: []string{"/path1", "/path3"}}, {TxPaths: []string{"/path2"}}},
	}},
	{"should cap to max threads if specified threads is greater than available cores", endpoint, []string{"/path1", "/path2", "/path3", "/path4", "/path5"}, 123, 0, 5, benchmark.Recipe{
		Endpoint: endpoint,
		Duration: time.Duration(123) * time.Second,
		Amount:   0,
		Runs:     []benchmark.Run{{TxPaths: []string{"/path1", "/path5"}}, {TxPaths: []string{"/path2"}}, {TxPaths: []string{"/path3"}}, {TxPaths: []string{"/path4"}}},
	}},
	{"should run correctly when many more files than threads", endpoint, []string{"/path1", "/path2", "/path3", "/path4", "/path5"}, 123, 0, 2, benchmark.Recipe{
		Endpoint: endpoint,
		Duration: time.Duration(123) * time.Second,
		Amount:   0,
